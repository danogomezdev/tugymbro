...
const estadoStyles = {
  ...
  cancelada: 'bg-neutral-900 text-neutral-500',
    ...
    <div className="min-h-screen bg-black pb-20">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="card flex items-center justify-between mb-4">
          <button onClick={() => navegarMes(-1)} className="text-neutral-400 hover:text-white"><ChevronLeft size={18} /></button>
          <span className="font-semibold text-white capitalize">{nombreMes}</span>
          <button onClick={() => navegarMes(1)} className="text-neutral-400 hover:text-white"><ChevronRight size={18} /></button>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          {[
            ...
            ['Canceladas', reservas.filter(r => r.estado === 'cancelada').length, 'text-neutral-500'],
        ...
        {cargando ? (
          ...
          <div className="card text-center py-10">
            <Calendar className="text-neutral-700 mx-auto mb-2" size={36} />
            <p className="text-neutral-500 text-sm">No hay reservas en {nombreMes}</p>
          </div>
        ) : (
            ...
            {reservas.map(r => (
              <div key={r.id} className="card flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      <p className="text-neutral-400 text-xs">{r.hora_inicio?.slice(0,5)} - {r.hora_fin?.slice(0,5)}</p>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {r.estado === 'confirmada' && (
                    <button onClick={() => cancelar(r.id)} className="text-neutral-600 hover:text-red-400 transition-colors"><XCircle size={16} /></button>
                  )}
                </div>
              </div>
            ))}
