...
const loginGym = async (req, res) => {
  ...
  try {
    ...
    if (gym.estado !== 'activo') return res.status(403).json({ error: 'Este gimnasio no está activo' });
    ...
    if (userResult.rows.length === 0) return res.status(401).json({ error: 'Email o contraseña incorrectos' });

    const usuario = userResult.rows[0];
    if (usuario.bloqueado) return res.status(403).json({ error: 'Tu cuenta está bloqueada. Contactá al gimnasio.', bloqueado: true });

    const token = jwt.sign(
      { id: usuario.id, email: usuario.email, nombre: usuario.nombre, apellido: usuario.apellido,
        rol: usuario.rol, plan: usuario.plan, gimnasio_id: gym.id, gym_slug: gym.slug, bloqueado: usuario.bloqueado },
    ...
    res.json({
      ...
      usuario: {
        ...
        bloqueado: usuario.bloqueado, fecha_vencimiento_pago: usuario.fecha_vencimiento_pago,
        debe_cambiar_password: usuario.debe_cambiar_password || false,
        gimnasio_id: gym.id, gym_slug: gym.slug
      },
      gimnasio: {
        id: gym.id, slug: gym.slug, nombre: gym.nombre,
        logo_url: gym.logo_url, color_primario: gym.color_primario, plan_nombre: gym.plan_nombre,
        features: {
          reservas: gym.feature_reservas, pagos: gym.feature_pagos,
          notificaciones: gym.feature_notificaciones, rutinas: gym.feature_rutinas,
...
const registroCliente = async (req, res) => {
  ...
  try {
    const gymResult = await pool.query(`SELECT id, estado FROM gimnasios WHERE slug = $1`, [gymSlug]);
    if (gymResult.rows.length === 0) return res.status(404).json({ error: 'Gimnasio no encontrado' });
    if (gymResult.rows[0].estado !== 'activo') return res.status(403).json({ error: 'Gimnasio no activo' });
    const gymId = gymResult.rows[0].id;
    const existe = await pool.query(`SELECT id FROM usuarios WHERE email = $1 AND gimnasio_id = $2`, [email, gymId]);
...
const loginSuperAdmin = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM superadmins WHERE email = $1 AND activo = true AND password_hash = crypt($2, password_hash)`,
      [email, password]
    );
...
// Acepta tanto camelCase (passwordActual) como snake_case (password_actual)
const cambiarPassword = async (req, res) => {
  const passwordActual = req.body.password_actual || req.body.passwordActual;
  const passwordNueva  = req.body.password_nueva  || req.body.passwordNueva;
